...
def convert_orderbook_to_dataframe(orderbook: Orderbook, n_levels: int = 10):
    ...
    for direction in ["buy", "sell"]:
        prices = reversed(orderbook[direction]) if direction == "buy" else orderbook[direction]  # type: ignore
        for level, price in enumerate(prices):
            if level >= n_levels:
                break
            total_volume = sum(order.volume for order in orderbook[direction][price])  # type: ignore
